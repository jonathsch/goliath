...
from ca_code.utils.image import linear2srgb, make_image_grid_batched, scale_diff_image, scale_img_chw
...
class AutoEncoder(nn.Module):
    ...
    def render(self, K: th.Tensor, Rt: th.Tensor, preds: Dict[str, Any]):
        ...
        valid_mask = self.geo_fn.valid_mask
        neck_mask = self.geo_fn.neck_mask.unsqueeze(-1)

        mask = th.logical_and(valid_mask, neck_mask)
        ...
        for b in range(B):
            render_output = render_gs(
                ...
                cy=K[b, 1, 2].item(),
                Rt=Rt[b],
                primpos=preds["primpos"][b][mask.view(-1)],
                primqvec=preds["primqvec"][b][mask.view(-1)],
                primscale=preds["primscale"][b][mask.view(-1)],
                opacity=preds["opacity"][b][mask.view(-1)],
                colors=preds["color"][b][mask.view(-1)],
                return_depth=True,
            )
...
class PrimDecoder(nn.Module):
    ...
    def __init__(
        ...
        self.vnocond_mod = nn.Sequential(
            ...
            *make_conv_trans(32, vind_ch, 4, 2, 1, "wn", ub=(512, 512)),
        ...
        self.vcond_mod = nn.Sequential(
            ...
            *make_conv_trans(128, 64, 4, 2, 1, "wn", nn.LeakyReLU(0.2, inplace=True), ub=(128, 128)),
            *make_conv_trans(64, 32, 4, 2, 1, "wn", nn.LeakyReLU(0.2, inplace=True), ub=(256, 256)),
            *make_conv_trans(32, vd_ch, 4, 2, 1, "wn", ub=(512, 512)),
        ...
        rgb = scale_img_chw(rgb, (self.slabsize, self.slabsize))
