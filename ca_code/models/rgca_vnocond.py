...
class PrimDecoder(nn.Module):
    ...
    def __init__(
        ...
        vind_ch = self.n_diff_coeffs + 11 + 1 + 3 + 1  # diffuse_sh + Gaussian params + roughness + spec_nml + spec_vis
        # vd_ch = 1  # specular visibility
    ...
    def forward(
        ...
        # f_vcond = f_vcond.permute(0, 2, 3, 1).view(B, -1, 1)
        ...
        # roughness
        sigma = f_vnocond[:, self.n_diff_coeffs + 11 : self.n_diff_coeffs + 12]
        sigma = sigma.permute(0, 2, 3, 1).view(B, -1)
        sigma = (th.exp(sigma) * 0.1).clamp(min=0.01)

        # view-dependent specular visibility
        spec_vis = th.sigmoid(f_vnocond[:, self.n_diff_coeffs + 12 : self.n_diff_coeffs + 13]).permute(0, 2, 3, 1).view(B, -1, 1)

        # specular normal
        spec_dnml = f_vnocond[:, self.n_diff_coeffs + 13 : self.n_diff_coeffs + 16]
        spec_dnml = spec_dnml.permute(0, 2, 3, 1).view(B, -1, 3)  # [B, n_prims, 3]
